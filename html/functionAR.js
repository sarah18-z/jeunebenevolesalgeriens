...
function validation(){
 ...
 /*   if(email.indexOf("@") == -1 || email.length < 10){
      swal ( "Oops" ,  "رجاءا أدخل بريدك الإلكتروني" ,  "error" ); 
      return false;
    }
    if(isNaN(tel) || tel.length != 10){
      swal ( "Oops" ,  "يرجى إدخال رقم الهاتف الخاص بك" ,  "error" ); 
      return false;
    }
    if(isNaN(tel1) || tel1.length != 10){
      swal ( "Oops" ,  "الرجاء إدخال رقم هاتف الطوارئ" ,  "error" ); 
      return false;
    }
    else{
      swal("أنت مسجل !", "سوف تتلقى إيمايل للتأكيد ", "success");
